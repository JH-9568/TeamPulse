...
- `open dist/OpenBrief.app`로 실행 테스트해줘.
- `.app`은 OpenBrief 본체를 포함한 완전한 설치 파일이 아니라, 설치된 `openbrief start --daemon`을 실행하고 로컬 대시보드를 여는 launcher라는 점을 설명해줘.
...
이 launcher는 새 앱 번들이지만 OpenBrief 본체를 포함하지 않습니다. 내부적으로 설치된 `openbrief start --daemon`을 실행하고 로컬 대시보드를 엽니다. 기본 주소는 `http://127.0.0.1:8000/dashboard`이며, 8000 포트가 사용 중이면 다른 포트로 실행될 수 있습니다.
